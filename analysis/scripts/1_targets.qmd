...
#| include: false
#| echo: false
# Unit tests (testthat): data and model checks; returns tibble with test name and passed/failed
...
#| include: false
#| echo: false
# Track 1_targets.qmd so project_unit_tests invalidates when the report is edited (e.g. title change)
tar_target(qmd_file, {
...
#| include: false
#| echo: false
# Project unit tests: compliance with project rules (structure, Cursor, Quarto, git, config)
...
<details><summary>Project unit tests</summary>

```{r}
#| echo: false
tryCatch({
  setwd(root_dir)
  pt <- tar_read(project_unit_tests)
  ...
  cat("Project unit tests not available. Run the pipeline (tar_make) first.\n")
})
```
...
tryCatch({
  ...
  cat("Unit tests not available. Run the pipeline (tar_make) first.\n")
})
```
...
tryCatch({
  ...
  unit_tests_tbl <- tar_read(unit_tests)
  project_unit_tests_tbl <- tar_read(project_unit_tests)
  tables_path <- file.path(tables_dir, "1_targets_report_tables.xlsx")
  writexl::write_xlsx(list(Summary = summary_tbl, Unit_tests = unit_tests_tbl, Project_unit_tests = project_unit_tests_tbl), tables_path)
}, error = function(e) {
