...
<details><summary>Report unit checks</summary>

```{r}
#| echo: false
tryCatch({
  setwd(root_dir)
  pt_setup <- tar_read(setup_unit_tests)
  pt_analysis <- tar_read(unit_tests)
  final_tests <- dplyr::bind_rows(
  ...
  final_tests %>% dplyr::select(test, Passed = passed_html) %>%
    knitr::kable(format = "html", escape = FALSE, table.attr = 'class="table" style="table-layout:fixed; width:100%; max-width:100%"')
}, error = function(e) {
  cat("Report unit checks not available. Run the pipeline (tar_make) first.\n")
})
```
