...
#| include: false
#| echo: false
# DAG for structural equations: temp → wind; temp, wind → log_ozone
tar_target(dag, {
  ggdag_status(
    dagify(
      wind ~ temp,
...
tar_target(setup_unit_tests, {
  ...
  config_tests <- list(
    run_one("README: mentions pipeline/run", testthat::expect_true(grepl("pipeline|quarto|run|quick start", read_txt(p("README.md")), ignore.case = TRUE))),
    ...
    run_one("Project rules: project-rules.mdc", {
      testthat::expect_true(file.exists(p(".cursor", "rules", "project-rules.mdc")))
      testthat::expect_true(grepl("Quarto|analysis|Cursor", read_txt(p(".cursor", "rules", "project-rules.mdc"))))
    }),
...
#| include: false
#| echo: false
# Report unit tests: summary of setup and analysis unit tests (for display in Report unit tests section)
tar_target(report_unit_tests, {
  pt_setup <- setup_unit_tests
  pt_analysis <- analysis_unit_tests
  dplyr::bind_rows(
...
tryCatch({
  ...
  analysis_unit_tests_tbl <- tar_read(analysis_unit_tests)
...
tryCatch({
  d <- tar_read(dag)
  ggplot2::ggsave(file.path(plot_dir, "causal_dag.png"), d, width = 9, height = 3, dpi = 600)
  d
}, error = function(e) {
...
Figure 1: Causal diagram (DAG) for the structural equations: temperature causally affects wind; temperature and wind affect log(ozone). Arrows indicate the direction of the hypothesized effects.
